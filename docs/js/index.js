...
function createIntersectionObserver() {
    ...
    intersectionObserver = new IntersectionObserver(entries => {
        ...
        for (const imageSeries of entries.filter(entry => entry.isIntersecting).map(entry => entry.target)) {
 ...
 */
function addImageSeries(imageSeries, i, interactionObserver) {
 ...
 */
function createTitle(imageSeries, i, rowsDiv) {
...
function createRow(row) {
    ...
    if (row.length === 1)
        rowDiv.classList.add("singleton");
