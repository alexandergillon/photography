...
async function validateImageSeries(manifestLength: number, index: number, imageName: string, imagePath: string) {
  ...
  assertBytesEqual(await r2Client.getByteArray(rowA[1].key), imageBytes, "png", `Add image series test: ${rowA[1].key}`)
  assertBytesEqual(await r2Client.getByteArray(rowA[2].key), imageBytes, "png", `Add image series test: ${rowA[2].key}`)
  assertBytesEqual(await r2Client.getByteArray(rowB[0].key), imageBytes, "png", `Add image series test: ${rowB[0].key}`)
  assertBytesEqual(await r2Client.getByteArray(rowB[1].key), imageBytes, "png", `Add image series test: ${rowB[1].key}`)
...
test('Adding image series works correctly when no manifest exists', async () => {
...
test('Adding image series works correctly when manifest exists', async () => {
