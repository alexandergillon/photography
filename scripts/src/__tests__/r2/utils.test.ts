...
test('Standard object key generation is correct', () => {
  expect(objectKey(uuid, 'series', 'image'))
    .toBe(`${uuid}_series/image`)
})

test('Object key generation with special characters is correct', () => {
  expect(objectKey(uuid, 'series:with!special@characters', 'image[with]special#characters'))
    .toBe(`${uuid}_series-with-special-characters/image-with-special-characters`)
})
...
test('Object key parsing works', () => {
  expect(parseObjectKey(`${uuid}_series/image`))
    .toEqual({ seriesUuid: uuid, seriesName: 'series', imageName: 'image' })
})
